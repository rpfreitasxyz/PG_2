...
kable(bp %>% select(1:7),
  caption = "Balanços patrimoniais da Copel, em milhões de reais. Base contábil de 2019 a 2014.",
  format = "latex",
  booktabs = TRUE,
  longtable = TRUE
  ) %>%
...
kable(bp %>% select(c(1, 7:12)),
  caption = "Balanços patrimoniais da Copel, em milhões de reais. Base contábil de 2014 a 2009.",
  format = "latex",
  booktabs = TRUE,
  longtable = TRUE
  ) %>%
...
bp_trat <- bp %>%
  ...
  mutate(Conta = as_factor(Conta)) %>%
  .[order(match(.$Conta, ordem)),] %>%
  select(-2) %>%
  drop_na() %>%
...
kable(bp_trat %>% select(1:6),
  ...
  # Indentacao do PL
  add_indent(c(75, 82)-2) %>%
  group_rows(index = c("Ativos" = 44, "Liabilities" = 25, "Shareholders' Equity" = 13))

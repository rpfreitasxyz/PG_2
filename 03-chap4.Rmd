...
É relevante comentar que, ao longo dos 10 anos, o patrimônio líquido da Copel não caiu de forma consolidada em qualquer ano; demonstra inclusive uma tendência de crescimento, equivalente a `r paste(round(100 * evo_patrim_liq, 2), "%", sep = "")` relativo a 2012. Isso pode ser expandido para as contas de ativos e passivos, o que é um indicador de estabilidade e previsibilidade de rendimentos.
...
dfc <- read_csv2("data/DFC_Copel.csv") %>%
  ...
  mutate_at(2:ncol(.), function(x) replace_na(x, "-") %>% str_replace("-", " ") %>% str_trim(side = "both")) %>%
  slice(1:(n()-2)) %>%
  select(1:6)
...
Conforme comentado no preâmbulo anterior, é razoável assumir que esse crescimento de lucro líquido no DRE – mais de 100%, comparando 2021 a 2019 – não é sustentável. Ainda assim, espera-se que a Copel mantenha um crescimento natural de receitas líquidas e pague dívidas, ou faça novos investimentos com o caixa adquirido dessas operações não-recorrentes. Assim, para fins desse estudo, considera-se que o lucro líquido se mantenha o mesmo em 2022, relativo ao ano de 2021. 

**!!REPENSAR AQUI!!**
Dessa forma, pressupõe-se um valor de EBIT de 3.1 bilhões de reais para o ano de 2021.
...
No que tange à sensibilidade do ativo perante o mercado, através de uma visita ao Yahoo! Finance[^BetaCPLE], temos que $\beta$ vale 0.34. Este foi calculado através de base mensal com duração de 5 anos.
...
WACC &= `r last(est_cap$cap_proprio)`*`r 0.0925 + 0.34 * 0.0721` + (1-0.34)*`r 1-last(est_cap$cap_proprio)`*0.085 \\
WACC &= `r last(est_cap$cap_proprio)*(0.0925 + 0.34 * 0.0721)` + `r (1-0.34)*(1-last(est_cap$cap_proprio))*0.085` \\
WACC &= `r last(est_cap$cap_proprio)*(0.0925 + 0.34 * 0.0721) + (1-0.34)*(1-last(est_cap$cap_proprio))*0.085`
\end{align*}

Dessa forma, a taxa de desconto para o DCF é `r paste(round(100 * (last(est_cap$cap_proprio)*(0.0925 + 0.34 * 0.0721) + (1-0.34)*(1-last(est_cap$cap_proprio))*0.085), 2), "%", sep = "")`. 

Para a taxa de crescimento, será considerado o crescimento da receita líquida, que, para empresas à base de concessões, representa uma taxa de crescimento sustentável oriunda majoritariamente de reajustes e revisões na tarifa aplicada aos clientes. Será considerada que a taxa de crescimento será mantida para os próximos 5 anos, em seguida a empresa teria um crescimento similar à taxa de crescimento médio da economia, tendo em vista que o setor elétrico possui uma causalidade de consumo de energia total para o PIB [@gadelha2014], embora não seja descartada a possibilidade de uma bidirecionalidade [@neto2016].^[Isso de certa forma assume que as concessões estariam "saturadas". Não necessariamente é o caso, podendo a Copel competir seja em geração, transmissão ou distribuição; essa consideração foi feita para fins deste estudo apenas e, para um caso mais aprofundado em que fosse feito um _valuation_ do tipo "soma das partes", seria necessário realizar essa comprovação para cada um dos negócios da Companhia.]

Conforme estabelecido pelo site do FMI,[^pibFMI] é razoável assumir que o PIB real brasileiro crescerá em 2% a.a. Dessa forma, no DCF:
